...
export class StudentAnswerQuestionsComponent implements OnInit {
  ...
  getAnswers(): void {
    this.api
      .GetStudentQuestions(this.student.id, this.quizId)
      .subscribe((q) => {
  ...
  getStudentMarks(): void {
    this.questionCount = this.questions.length;
    this.correctAnswers = this.studentQuestion.filter(
      (x) => x.IsCorrectAnswer == true
    );
    this.studentMarks = Math.floor(
      (this.correctAnswers.length / this.questionCount) * 100
    );
  }
}
